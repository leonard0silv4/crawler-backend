  ...
  async cronUsers() {
    try {
      ...
      for (const user of users) {
        ...
        if (
          scheduledCrons[user._id] &&
          scheduledCrons[user._id].cronInterval === cronInterval
        ) {
          console.log(
            `Cron já agendado para o usuário ${user._id} com o mesmo cronInterval. Nenhuma ação necessária.`
          );
          continue;
        }
        ...
        console.log(
          `Novo cron agendado para o usuário ${user._id} com tempo ${cronInterval}.`
        );
      }

    ...
    while (tentativaAtual <= maxRetries) {
      try {
        ...
        const seller = $(
          ...
          .text();
        const buyActive = $(
          '[formaction="https://www.mercadolivre.com.br/gz/checkout/buy"]'
        ).length;
        const full = $('[href="#full_icon"]').length ? true : false;
        const catalog = $(".ui-pdp-other-sellers-item__buybox").length
          ? true
          : false;
        const paused = $(
          ".ui-pdp-message.ui-vpp-message .andes-message__content"
        ).length
          ? true
          : false;
